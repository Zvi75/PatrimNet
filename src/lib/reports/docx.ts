        ...
        children: items.map(
          (item) =>
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [new TextRun({ text: item.label, size: 16, color: "94A3B8" })],
                }),
                new Paragraph({
                  children: [
                    new TextRun({ text: item.value, bold: true, size: 22, color: "0F172A" }),
                  ],
                }),
              ],
            }),
        ...
        children: headers.map(
          (h) =>
            new TableCell({
              ...
              children: [
                new Paragraph({
                  children: [new TextRun({ text: h, bold: true, size: 18, color: "475569" })],
                }),
              ],
            }),
        ),
...
function footer(): Paragraph {
  return new Paragraph({
    children: [
      new TextRun({
        text: `PatrimNet · Généré le ${new Date().toLocaleDateString("fr-FR")} · Document confidentiel`,
        color: "CBD5E1",
        size: 16,
      }),
    ],
    ...
    sections: [
      {
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `Coût total des intérêts : ${formatCurrency(totalInterest)} · Durée : ${d.schedule.length} mois`,
                size: 18,
                color: "475569",
              }),
            ],
