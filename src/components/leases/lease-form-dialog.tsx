...
export function LeaseFormDialog({ assets, tenants, lease, trigger, defaultOpen = false, onClose }: LeaseFormDialogProps) {
  ...
  const canSubmit = form.reference && form.assetId && form.tenantId && form.startDate && form.endDate && form.baseRent;
          ...
          <form id="lease-form" onSubmit={handleSubmit} className="space-y-4 pb-2">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input id="l-ref" value={form.reference} onChange={(e) => set("reference")(e.target.value)} placeholder="BAIL-2024-001" required autoFocus />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="l-start" type="date" value={form.startDate} onChange={(e) => set("startDate")(e.target.value)} required />
              </div>

              <div className="space-y-2">
                <Label htmlFor="l-end">Date de fin *</Label>
                <Input id="l-end" type="date" value={form.endDate} onChange={(e) => set("endDate")(e.target.value)} required />
              </div>

              <div className="space-y-2">
                <Label htmlFor="l-rent">Loyer de base (€/mois) *</Label>
                <Input id="l-rent" type="number" min="0" step="0.01" value={form.baseRent} onChange={(e) => set("baseRent")(e.target.value)} placeholder="1500" required />
              </div>

              <div className="space-y-2">
                <Label htmlFor="l-charges">Charges (€/mois)</Label>
                <Input id="l-charges" type="number" min="0" step="0.01" value={form.charges} onChange={(e) => set("charges")(e.target.value)} placeholder="150" />
              </div>

              <div className="space-y-2">
                <Label>Indexation</Label>
                <Select value={form.indexationIndex || "none"} onValueChange={(v) => set("indexationIndex")(v === "none" ? "" : v)}>
                  <SelectTrigger><SelectValue placeholder="—" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">—</SelectItem>
                    {INDEXATION_INDEXES.map((i) => <SelectItem key={i} value={i}>{i}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="l-revision" type="date" value={form.nextRevisionDate} onChange={(e) => set("nextRevisionDate")(e.target.value)} />
              </div>
              ...
              <div className="flex items-center gap-2 pt-6">
                ...
                />
                <Label htmlFor="l-tva" className="cursor-pointer">TVA applicable</Label>
              </div>
            </div>
          </form>
        </ScrollArea>
